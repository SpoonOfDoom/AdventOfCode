...
namespace AdventOfCode.Days._2017
{
    ...
    class Day01 : Day
	{
	    ...
	    {
	        int sum = 0;
	        int halfWayAmount = digits.Count / 2;
	        for (int i = 0; i < digits.Count; i++)
	        {
	            int next;
	            if (part2Mode)
	            {
	                next = (i + halfWayAmount) % digits.Count;
                }
	            else
	            {
	                next = (i + 1) % digits.Count;
                }
	    ...
	    private int GetDigitSum(string digitString, bool part2Mode = false)
	    {
	        return GetDigitSum(digitString.Select(c => c.ToInt()).ToList(), part2Mode);
	    }
	    ...
	    protected override object GetSolutionPart1()
	    {
             ...
             * --- Day 1: Inverse Captcha ---
                ...
                By the time you make it to sub-basement 17, there are only a few minutes until midnight. "We have a big problem," she says; "there must be almost fifty bugs
                in this system, but nothing else can print The List. Stand in this square, quick! There's no time to explain; if you can convince them to pay you in stars,
             ...
             */
            #region Testrun
            Dictionary<string, int> testInputs = new Dictionary<string, int>
                                                 {
                                                    {"1122", 3 },
                                                    {"1111", 4 },
                                                    {"1234", 0 },
	        ...
	        foreach (KeyValuePair<string, int> pair in testInputs)
	        {
	            int testResult = GetDigitSum(pair.Key);
	            if (testResult != pair.Value)
	            {
	                throw new Exception($"Test failed! Expected: {pair.Value}, Actual: {testResult}");
	            }
	        }
            #endregion
	    ...
	    protected override object GetSolutionPart2()
	    {
	        ...
	        foreach (KeyValuePair<string, int> pair in testInputs)
	        {
	            int testResult = GetDigitSum(pair.Key, part2Mode: true);
	            if (testResult != pair.Value)
	            {
	                throw new Exception($"Test failed! Expected: {pair.Value}, Actual: {testResult}");
	            }
	        ...
	        int result = GetDigitSum(Input, part2Mode: true);
            return result;
	    }
	}
}
